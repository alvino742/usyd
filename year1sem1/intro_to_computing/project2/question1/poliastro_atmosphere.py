...
All of the following code has been copied from the poliastro package.
"""
...
class COESA:
    ...
    def _get_index(self, x, x_levels):
        ...
        """
        x = x.to_value(u.km)
        x_levels = np.array((x_levels << u.km).value)
        diff = x_levels - x
        i = np.argmax(np.where(diff <= 0, diff, -np.inf))
        return i
        # x = x.to_value(u.km)
...
class COESA76(COESA):
    ...
    def density(self, alt, geometric=True):
        ...
        # If it's too high, just return zero
        if z > self.zb_levels[-1]:
            return 0 * u.kg / u.m**3

        # If it's too low, keep it at sea level
        if z <= self.zb_levels[0]:
            return 1.2250 * u.kg / u.m**3
        ...
        if z <= 86 * u.km:
            ...
            # Solve the polynomial
            zval = z.to_value(u.km)
            rho = (
                np.exp(A * zval**4 + B * zval**3 + C * zval**2 + D * zval + E)
                * u.kg
                / u.m**3
            )
